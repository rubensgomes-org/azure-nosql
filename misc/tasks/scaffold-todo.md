...
`azure-nosql` was scaffolded from `spring-blueprint` as build/CI/release machinery
only. This file records what was deliberately left outstanding so the scaffold
could be handed over green.

## Outstanding

- [ ] Add the Azure dependency — spring-cloud-azure-starter-data-cosmos — to `app/build.gradle.kts`,
...
- [ ] Build the real endpoint that demonstrates Azure Cosmos DB.
...
- [ ] Create the SonarCloud project `rubensgomes-org_azure-nosql` in the
