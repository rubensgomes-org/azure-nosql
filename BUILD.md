...
How the Gradle build for `azure-nosql` is put together, what each task
...
> **Note** — `logging.level.com.rubensgomes.azure.nosql` is set to `trace` in the
...
```bash
kill $(pgrep -f com.rubensgomes.azure.nosql.App)
```
...
```bash
./gradlew bootJar
java -jar app/build/libs/azure-nosql-<version>.jar
```
...
```kotlin
excludes = listOf("com.rubensgomes.azure.nosql.config.*")
```
...
| Archive                                  | Contents                                                 |
|------------------------------------------|----------------------------------------------------------|
| `azure-nosql-<version>.jar`         | executable Spring Boot jar (layered, for Docker caching) |
| `azure-nosql-<version>-plain.jar`   | library jar, classes only                                |
| `azure-nosql-<version>-sources.jar` | sources                                                  |
| `azure-nosql-<version>-javadoc.jar` | Javadoc                                                  |
...
| `sonar.projectKey`       | `rubensgomes-org_azure-nosql` |
| `sonar.projectName`      | `azure-nosql`                 |
...
there is no `azure-nosql-spring-boot.jar` for the Dockerfile and no `.env`.
